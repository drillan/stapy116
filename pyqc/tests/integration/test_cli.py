...
class TestCheckCommand:
    ...
    def test_check_single_file_success(self, mock_run: Mock, runner: CliRunner, sample_python_file: Path) -> None:
    ...
    @patch("subprocess.run")
    def test_check_single_file_with_issues(self, mock_run: Mock, runner: CliRunner, sample_python_file: Path) -> None:
    ...
    @patch("subprocess.run")
    def test_check_directory(self, mock_run: Mock, runner: CliRunner, sample_project: Path) -> None:
    ...
    @patch("subprocess.run")
    def test_check_json_output(self, mock_run: Mock, runner: CliRunner, sample_python_file: Path) -> None:
        ...
        result = runner.invoke(app, ["check", str(sample_python_file), "--output", "json"])
    ...
    @patch("subprocess.run")
    def test_check_github_output(self, mock_run: Mock, runner: CliRunner, sample_python_file: Path) -> None:
        ...
        result = runner.invoke(app, ["check", str(sample_python_file), "--output", "github"])
...
class TestFixCommand:
    ...
    @patch("subprocess.run")
    def test_fix_single_file(self, mock_run: Mock, runner: CliRunner, sample_python_file: Path) -> None:
    ...
    @patch("subprocess.run")
    def test_fix_dry_run(self, mock_run: Mock, runner: CliRunner, sample_python_file: Path) -> None:
        """Test dry run mode."""
        # Mock ruff format (successful fix)
        mock_run.return_value = subprocess.CompletedProcess(
            args=["ruff", "format"],
