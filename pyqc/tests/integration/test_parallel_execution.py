...
class TestParallelExecution:
    ...
    def test_parallel_check_multiple_files(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
    ...
    @patch("subprocess.run")
    def test_sequential_vs_parallel_performance(self, mock_run: Mock, temp_project: Path) -> None:
    ...
    @patch("subprocess.run")
    def test_parallel_execution_with_failures(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
        """Test parallel execution handles failures gracefully."""
        # Mock some files to fail
        def mixed_subprocess(*args, **kwargs):
        ...
        assert len(error_result.issues) >= 1  # May have multiple issues from different checkers
    ...
    @patch("subprocess.run")
    def test_parallel_fix_multiple_files(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
    ...
    @patch("subprocess.run")
    def test_parallel_execution_exception_handling(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
        """Test parallel execution handles exceptions properly."""
        # Mock subprocess to raise exception for one file
        def exception_subprocess(*args, **kwargs):
...
class TestPerformanceMetrics:
    ...
    @patch("subprocess.run")
    def test_performance_metrics_calculation(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
    ...
    @patch("subprocess.run")
    def test_performance_metrics_with_failures(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
...
class TestWorkerConfiguration:
    ...
    @patch("subprocess.run")
    def test_custom_max_workers(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
    ...
    @patch("subprocess.run")
    def test_auto_worker_scaling(self, mock_run: Mock, temp_project: Path, config_parallel: PyQCConfig) -> None:
