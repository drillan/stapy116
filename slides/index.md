...
````{container} custom
```{literalinclude} ../.claude/pyqc-spec.md
```
````
...
- 記録への投資はClaude Codeの自律的な能力を最大限に引き出す
...
- Write, Edit, MultiEditツール使用時に自動実行
- 編集されたファイルに対してのみ品質チェック
- 非ブロッキング実行で開発を妨げない
...
```json
{
  "hooks": {
    "PostToolUse": {
      "Write,Edit,MultiEdit": "uv run ruff check ${file} --output-format github"
    }
  }
}
```
...
リアルタイム vs コミット時の品質チェック

- タイミング: ファイル編集直後 vs コミット前
- 対象範囲: 編集ファイルのみ vs 全ての変更ファイル
- フィードバック: 即座 vs コミット時
...
- 堅牢なコードへの投資はClaude Codeの自律性と安全性を両立するための要素
- ツールを活用し、属人性を回避
